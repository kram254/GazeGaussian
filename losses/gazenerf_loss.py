...
class GazeNeRFLoss(object):
    ...
    def calc_orthogonality_loss(self, gaze_features, head_features, expr_features):
        gaze_norm = F.normalize(gaze_features, dim=-1)
        head_norm = F.normalize(head_features, dim=-1)
        expr_norm = F.normalize(expr_features, dim=-1)
        
        gaze_head_sim = torch.abs((gaze_norm * head_norm).sum(dim=-1))
        gaze_expr_sim = torch.abs((gaze_norm * expr_norm).sum(dim=-1))
        head_expr_sim = torch.abs((head_norm * expr_norm).sum(dim=-1))
        
        orth_loss = (gaze_head_sim + gaze_expr_sim + head_expr_sim).mean()
        return orth_loss
    ...
    def calc_data_loss(
        ...
        masked_res_img_pro = res_img_pro * head_mask_c3b + 1 - head_mask_c3b
        masked_res_img_eyes_pro = res_img_eyes_pro * eyes_mask_c3b + 1 - eyes_mask_c3b
        masked_res_img_face_pro = res_img_face_pro * face_mask_c3b + 1 - face_mask_c3b

        if self.use_l1_loss:
            head_loss = F.l1_loss(masked_res_img, masked_gt_rgb) 
            head_pro_loss = F.l1_loss(masked_res_img_pro, masked_gt_rgb)
            eyes_loss = F.l1_loss(
                masked_res_img_eyes, masked_gt_rgb_eyes
            )
            eye_pro_loss = F.l1_loss(
                masked_res_img_eyes_pro, masked_gt_rgb_eyes
            )
            face_loss = F.l1_loss(
                masked_res_img_face, masked_gt_rgb_face
            )
            face_pro_loss = F.l1_loss(
                masked_res_img_face_pro, masked_gt_rgb_face
            )
        else:
            head_loss = F.mse_loss(masked_res_img, masked_gt_rgb)
            head_pro_loss = F.mse_loss(masked_res_img_pro, masked_gt_rgb)
            eyes_loss = F.mse_loss(masked_res_img_eyes, masked_gt_rgb_eyes)
            eye_pro_loss = F.mse_loss(masked_res_img_eyes_pro, masked_gt_rgb_eyes)
            face_loss = F.mse_loss(masked_res_img_face, masked_gt_rgb_face)
            face_pro_loss = F.mse_loss(masked_res_img_face_pro, masked_gt_rgb_face)
        ...
        if self.use_ssim_loss:
            ...
            ssim_head_pro_loss = 0.1 * (1 - ssim(masked_res_img_pro, masked_gt_rgb))
            ssim_eyes_loss = 0.1 * (1 - ssim(masked_res_img_eyes, masked_gt_rgb_eyes))
            ssim_eyes_pro_loss = 0.1 * (1 - ssim(masked_res_img_eyes_pro, masked_gt_rgb_eyes))
            ssim_face_loss = 0.1 * (1 - ssim(masked_res_img_face, masked_gt_rgb_face))
            ssim_face_pro_loss = 0.1 * (1 - ssim(masked_res_img_face_pro, masked_gt_rgb_face))
        ...
        if self.use_vgg_loss and epoch >= self.vgg_loss_begin:
            ...
            vgg_face_pro_loss = self.vgg_loss_func(masked_res_img_face_pro, masked_gt_rgb_face, normalize=True).mean() * self.vgg_importance
            vgg_eyes_loss = self.vgg_loss_func(masked_res_img_eyes, masked_gt_rgb_eyes, normalize=True).mean() * self.vgg_importance
            vgg_eyes_pro_loss = self.vgg_loss_func(masked_res_img_eyes_pro, masked_gt_rgb_eyes, normalize=True).mean() * self.vgg_importance
            vgg_head_loss = self.vgg_loss_func(masked_res_img, masked_gt_rgb, normalize=True).mean() * self.vgg_importance
            vgg_head_pro_loss = self.vgg_loss_func(masked_res_img_pro, masked_gt_rgb, normalize=True).mean() * self.vgg_importance
    ...
    def calc_total_loss(
        ...
        if hasattr(self, 'use_orthogonality_loss') and self.use_orthogonality_loss:
            gaze_features = data['nl3dmm_para_dict']['pitchyaw']
            head_features = data['nl3dmm_para_dict']['pose'][:, :3]
            expr_features = data['nl3dmm_para_dict']['expr_code']
            
            orth_loss = self.calc_orthogonality_loss(gaze_features, head_features, expr_features)
            loss_dict["orthogonality_loss"] = orth_loss * self.orthogonality_loss_importance
            total_loss += loss_dict["orthogonality_loss"]
