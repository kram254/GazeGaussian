            ...
            for b in range(B):
                ...
                if self.is_eye:
                    ...
                    if self.is_rotate_eye:
                        left_dists, left_idx, _ = knn_points(xyz[b:b+1], self.landmarks_3d_neutral[:6].unsqueeze(0))
                        right_dists, right_idx, _ = knn_points(xyz[b:b+1], self.landmarks_3d_neutral[6:12].unsqueeze(0))
                        ...
                        left_mask = left_eye_weights[0].squeeze(-1) > 0.00
                        if left_mask.sum() > 0:
                            left_eye_deform_input = torch.cat([self.pos_embedding(xyz[b, left_mask].mean(dim=0, keepdim=True)), self.pos_embedding(data['nl3dmm_para_dict']['pitchyaw'][b:b+1])], 1).unsqueeze(-1)
                            ...
                            verts_batch_expanded = xyz[b:b+1, left_mask].unsqueeze(-1)
                            left_eye_deform = torch.matmul(left_rotation_matrices, verts_batch_expanded).squeeze(-1)
                            xyz[b, left_mask] = left_eye_deform[0]

                        right_mask = right_eye_weights[0].squeeze(-1) > 0.0
                        if right_mask.sum() > 0:
                            right_eye_deform_input = torch.cat([self.pos_embedding(xyz[b, right_mask].mean(dim=0, keepdim=True)), self.pos_embedding(data['nl3dmm_para_dict']['pitchyaw'][b:b+1])], 1).unsqueeze(-1)
                            ...
                            verts_batch_expanded = xyz[b:b+1, right_mask].unsqueeze(-1)
                            right_eye_deform = torch.matmul(right_rotation_matrices, verts_batch_expanded).squeeze(-1)
                            xyz[b, right_mask] = right_eye_deform[0]
