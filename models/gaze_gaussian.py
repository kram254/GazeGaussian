...
class GazeGaussianNet(nn.Module):
    def __init__(self, opt: BaseOptions,
        ...
        if hasattr(self.opt, 'neural_renderer_type') and self.opt.neural_renderer_type == "dit":
            self.neural_render = DiTNeuralRenderer(
                ...
                patch_size=self.opt.dit_patch_size,
                depth=self.opt.dit_depth,
                num_heads=self.opt.dit_num_heads,
                mlp_ratio=self.opt.dit_mlp_ratio,
                cond_dim=self.opt.shape_coeffs_dim if self.opt.unet_atten else None,
                use_vae=self.opt.use_vae if hasattr(self.opt, 'use_vae') else False,
                vae_z_channels=self.opt.vae_z_channels if hasattr(self.opt, 'vae_z_channels') else 4,
                freeze_vae=self.opt.freeze_vae if hasattr(self.opt, 'freeze_vae') else True,
                predict_noise=self.opt.dit_predict_noise if hasattr(self.opt, 'dit_predict_noise') else False,
            )
        else:
